...
const Navbar = () => {
  ...
  return (
    <nav className="fixed top-0 w-full flex items-center justify-between px-6 pl-1 py-2 shadow-md bg-white z-50">
      {/* Logo */}
      <img src={MySign} alt="Logo" className="h-15 w-65 rounded-full" />

      <span className='flex gap-4 items-center'>
        {/* Home Link */}
        <button onClick={() => smoothScroll('hero')} className="text-gray-800 font-medium md:hidden sm:inline-block">
          Home
        </button>
        ...
        <button className="hidden md:block bg-orange-500 text-white px-4 py-2 rounded-md hover:bg-orange-600 transition">
          <a href="https://drive.google.com/file/d/1BwWNUm8uoGfyk5-bbKbEHWuZfXEzpnp1/view?usp=drive_link">MY CV</a>
        </button>
      </span>
